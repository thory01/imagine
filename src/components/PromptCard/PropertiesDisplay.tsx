...
export const PropertiesDisplay: React.FC<PropertiesDisplayProps> = ({ prompt }) => {
    ...
    return (
        <div className="flex flex-wrap gap-2 mt-2">
            {displayProperties.map(({ key, label, extraText, link, joiner }) => {
                ...
                return (
                    <span key={String(key)} className="text-gray-700 text-[12px] font-medium bg-slate-200 px-2 rounded-sm">
                        {label}: {displayValue}{extraText || ""}
                        {link && (
